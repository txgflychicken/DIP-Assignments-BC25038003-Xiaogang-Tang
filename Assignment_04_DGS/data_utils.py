...
def sample_farthest_points(points, K):
    """Simple farthest point sampling in pure PyTorch (replaces pytorch3d).
    ...
    for k in range(1, K):
        last_idx = indices[:, k-1]  # (B,)
        last_pts = points[torch.arange(B), last_idx]  # (B, 3)
        d = torch.norm(points - last_pts.unsqueeze(1), dim=2)  # (B, N)
        dists = torch.minimum(dists, d)
        indices[:, k] = torch.argmax(dists, dim=1)
    return points[torch.arange(B).unsqueeze(1), indices], indices
...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Sample 3D points to a specific number using farthest point sampling
        if self.points3D_xyz.shape[0] > maximum_pts_num:
            _, indices = sample_farthest_points(self.points3D_xyz.unsqueeze(0), K=maximum_pts_num)
            self.points3D_xyz = self.points3D_xyz[indices.reshape(-1)]
            self.points3D_rgb = self.points3D_rgb[indices.reshape(-1)]
            print(f"Subsampled 3D points from {len(points3D)} to {maximum_pts_num}")
